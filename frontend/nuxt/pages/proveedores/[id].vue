  ...
  <div class="proveedor-page">
    ...
    <main class="page-content">
      ...
      <header class="page-header animate-fade-in-up">
        ...
        <div>
          <h1 class="page-title">{{ editando ? 'Editar proveedor' : 'Detalle proveedor' }}</h1>
          <p class="page-subtitle">{{ form.nombre_provee || 'Cargando...' }}</p>
        </div>
      ...
      <template v-else>
        ...
        <div class="logo-section animate-fade-in-up stagger-1">
          <div class="logo-wrap">
            <img
              :src="form.logo_provee || '/images/avatar_default.png'"
              :alt="form.nombre_provee"
              class="logo-img"
              @error="(e) => (e.target as HTMLImageElement).src = '/images/avatar_default.png'"
            />
          </div>
          <div class="logo-info">
            <span class="logo-cat">{{ form.cate_provee }}</span>
            <span class="logo-status" :class="form.activo_provee ? 'active' : 'inactive'">
              {{ form.activo_provee ? '● Activo' : '● Inactivo' }}
            </span>
          </div>
        </div>
        ...
        <div class="form-card animate-fade-in-up stagger-2">
          ...
          <div class="field-group">
            ...
            <input v-if="editando" v-model="form.nombre_provee" type="text" class="field-input" maxlength="100" />
            <p v-else class="field-value">{{ form.nombre_provee }}</p>
          </div>
          ...
          <div class="field-group">
            ...
            <div v-if="editando" class="select-wrapper">
              <select v-model="form.cate_provee" class="field-select">
            ...
            <p v-else class="field-value">{{ form.cate_provee }}</p>
          </div>

          <!-- Dirección -->
          <div class="field-group">
            <label class="field-label">Dirección</label>
            <input v-if="editando" v-model="form.direccion_provee" type="text" class="field-input" maxlength="200" />
            <p v-else class="field-value">{{ form.direccion_provee || '—' }}</p>
          </div>
          ...
          <div class="field-group">
            ...
            <input v-if="editando" v-model="form.te_provee" type="tel" class="field-input" maxlength="50" />
            <p v-else class="field-value">{{ form.te_provee || '—' }}</p>
          </div>

          <!-- Email -->
          <div class="field-group">
            <label class="field-label">Email</label>
            <input v-if="editando" v-model="form.email_provee" type="email" class="field-input" maxlength="100" />
            <p v-else class="field-value">{{ form.email_provee || '—' }}</p>
          </div>

          <!-- Representante -->
          <div class="field-group">
            <label class="field-label">Representante</label>
            <input v-if="editando" v-model="form.representa_provee" type="text" class="field-input" maxlength="100" />
            <p v-else class="field-value">{{ form.representa_provee || '—' }}</p>
          </div>

          <!-- URL Logo -->
          <div class="field-group">
            <label class="field-label">URL del logo</label>
            <input v-if="editando" v-model="form.logo_provee" type="url" class="field-input" />
            <p v-else class="field-value url-value">{{ form.logo_provee || '—' }}</p>
          </div>
          ...
          <div v-if="!editando" class="field-group">
            ...
            <p class="field-value">{{ form.fechaIngreso_provee || '—' }}</p>
          </div>
...
async function cargar() {
  ...
  errorCarga.value = ''
  try {
    const data = await api<any>(`/proveedores/${id}`)
    form.nombre_provee    = data.nombre_provee    || ''
    form.cate_provee      = data.cate_provee      || ''
    form.direccion_provee = data.direccion_provee || ''
    form.lat              = data.lat              ?? null
    form.lng              = data.lng              ?? null
    form.te_provee        = data.te_provee        || ''
    form.email_provee     = data.email_provee     || ''
    form.representa_provee = data.representa_provee || ''
    form.logo_provee      = data.logo_provee      || ''
    form.activo_provee    = data.activo_provee    ?? true
    form.fechaIngreso_provee = data.fechaIngreso_provee || ''
    formOriginal = { ...form }
  } catch (err: any) {
    errorCarga.value = err?.data?.detail || err?.message || 'No se pudo cargar el proveedor'
  } finally {
    loading.value = false
  }
}
...
async function guardar() {
  ...
  try {
    ...
    // Agregar lat/lng si existen para que el backend actualice ubicacion_provee
    if (form.lat !== null && form.lng !== null) {
      payload.lat = Number(form.lat)
      payload.lng = Number(form.lng)
    }

    await api(`/proveedores/${id}`, { method: 'PUT', body: payload })
    formOriginal = { ...form }
    editando.value = false
