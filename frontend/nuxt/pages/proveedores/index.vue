    ...
    <main class="page-content">
      <header class="page-header animate-fade-in-up">
        ...
        <div class="header-right">
          ...
          <button v-if="isAdmin" class="add-btn" @click="navigateTo('/proveedores/nuevo')">
      ...
      <div v-else-if="errorMsg" class="state-box animate-fade-in-up">
        ...
        <p>{{ errorMsg }}</p>
        <button class="retry-btn" @click="cargarProveedores">Reintentar</button>
      </div>

      <!-- LISTA -->
      <div v-else-if="proveedoresFiltrados.length > 0" class="proveedores-lista animate-fade-in-up stagger-3">
        <article
          v-for="p in proveedoresFiltrados"
          :key="p.id_provee"
          class="proveedor-card"
          @click="navigateTo(`/proveedores/${p.id_provee}`)"
        >
          <div class="proveedor-logo-wrap">
            <img
              :src="p.logo_provee || '/images/avatar_default.png'"
              :alt="p.nombre_provee"
          ...
          <div class="proveedor-info">
            <p class="proveedor-nombre">{{ p.nombre_provee }}</p>
            <p class="proveedor-dir">{{ p.direccion_provee }}</p>
            <div class="proveedor-meta">
              <span class="proveedor-cat">{{ p.cate_provee }}</span>
              <span v-if="p.distancia !== null && p.distancia !== undefined" class="proveedor-dist">
                📍 {{ formatDistancia(p.distancia) }}
              </span>
              <span v-else-if="sortMode === 'distancia'" class="proveedor-dist muted">Sin ubicación</span>
            </div>
          </div>
          <svg class="proveedor-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div v-else class="state-box animate-fade-in-up">
        ...
        <button v-if="isAdmin" class="retry-btn" @click="navigateTo('/proveedores/nuevo')">Agregar el primero</button>
      </div>
...
interface Proveedor {
  ...
  direccion_provee: string
  te_provee: string | null
  email_provee: string | null
  representa_provee: string
  cate_provee: string
...
const proveedoresConDistancia = computed(() =>
...
const proveedoresFiltrados = computed(() => {
  let list = [...proveedoresConDistancia.value]
  if (searchQuery.value.trim()) {
    const q = searchQuery.value.toLowerCase()
    list = list.filter(p =>
      p.nombre_provee.toLowerCase().includes(q) ||
      p.direccion_provee?.toLowerCase().includes(q) ||
      p.cate_provee?.toLowerCase().includes(q)
    )
  }
  if (sortMode.value === 'distancia' && userLocation.value) {
    list.sort((a, b) => (a.distancia ?? Infinity) - (b.distancia ?? Infinity))
  } else {
    list.sort((a, b) => a.nombre_provee.localeCompare(b.nombre_provee, 'es'))
  }
  return list
})
...
async function cargarProveedores() {
  ...
  errorMsg.value = ''
  try {
    const res = await api<{ count: number; results: Proveedor[] }>('/proveedores')
    proveedores.value = res.results || []
  } catch (err: any) {
    console.error('Error cargando proveedores:', err)
    errorMsg.value = err?.message || 'No se pudieron cargar los proveedores'
  } finally {
    loading.value = false
  }
}
...
/* ─── LISTA ─── */
.proveedores-lista { display:flex; flex-direction:column; gap:0.75rem; }
.proveedor-card {
...
.proveedor-logo { width:100%; height:100%; object-fit:cover; }
.proveedor-info { flex:1; min-width:0; }
.proveedor-nombre { color:var(--text-primary); font-size:0.95rem; font-weight:600; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.proveedor-dir { color:var(--text-secondary); font-size:0.78rem; margin:0.2rem 0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.proveedor-meta { display:flex; align-items:center; gap:0.75rem; }
.proveedor-cat { color:var(--text-muted); font-size:0.75rem; }
.proveedor-dist { color:var(--accent-gold); font-size:0.75rem; font-weight:500; }
.proveedor-dist.muted { color:var(--text-muted); }
.proveedor-arrow { color:var(--text-muted); flex-shrink:0; opacity:0.5; }
.proveedor-card:hover .proveedor-arrow { opacity:1; color:var(--accent-gold); }
