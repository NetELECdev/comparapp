    ...
    <main class="page-content">
      ...
      <div class="form-card animate-fade-in-up stagger-1">
        ...
        <div class="field-group">
          ...
          <input
            v-model="form.nombre_comer"
            type="text"
            class="field-input"
            :class="{ error: errors.nombre_comer }"
            placeholder="Ej: Supermercado Central"
            maxlength="100"
          />
          <span v-if="errors.nombre_comer" class="field-error">{{ errors.nombre_comer }}</span>
        </div>
        ...
        <div class="field-group">
          ...
          <div class="select-wrapper">
            <select v-model="form.cate_comer" class="field-select" :class="{ error: errors.cate_comer }">
          ...
          <span v-if="errors.cate_comer" class="field-error">{{ errors.cate_comer }}</span>
        </div>
        ...
        <div class="field-group">
          ...
          <input
            v-model="form.direccion_comer"
            type="text"
            class="field-input"
            :class="{ error: errors.direccion_comer }"
            placeholder="Ej: Av. San Martín 450"
            maxlength="200"
          />
          <span v-if="errors.direccion_comer" class="field-error">{{ errors.direccion_comer }}</span>
        </div>
        ...
        <div class="field-group">
          ...
          <input
            v-model="form.email_comer"
            type="email"
            class="field-input"
            :class="{ error: errors.email_comer }"
            placeholder="contacto@comercio.com"
            maxlength="100"
          />
          <span v-if="errors.email_comer" class="field-error">{{ errors.email_comer }}</span>
        </div>
        ...
        <div class="field-group">
          ...
          <input
            v-model="form.logo_comer"
            type="url"
            class="field-input"
            placeholder="https://..."
          />
          <!-- Preview del logo -->
          <div v-if="form.logo_comer" class="logo-preview">
            <img
              :src="form.logo_comer"
              alt="Preview logo"
        ...
        <div class="field-group">
          <label class="toggle-label">
            <div class="toggle-info">
              ...
              <span class="toggle-desc">El comercio será visible para los usuarios</span>
            </div>
      ...
      <div class="form-actions animate-fade-in-up stagger-2">
        <button class="btn-cancel" @click="navigateTo('/comercios')" :disabled="saving">
          Cancelar
        </button>
...
function validar(): boolean {
  ...
  if (!form.nombre_comer.trim())
    errors.nombre_comer = 'El nombre es obligatorio'

  if (!form.cate_comer)
    errors.cate_comer = 'Seleccioná una categoría'

  if (!form.direccion_comer.trim())
    errors.direccion_comer = 'La dirección es obligatoria'

  if (form.email_comer && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email_comer))
    errors.email_comer = 'El email no es válido'
...
async function guardar() {
  ...
  try {
    const payload = {
      ...
      te_comer:         form.te_comer.trim() || null,
      email_comer:      form.email_comer.trim() || null,
      representa_comer: form.representa_comer.trim() || '',
      logo_comer:       form.logo_comer.trim() || null,
      activo_comer:     form.activo_comer,
    }

    await api('/comercios', { method: 'POST', body: payload })

    successMsg.value = '✅ Comercio guardado correctamente'
    setTimeout(() => navigateTo('/comercios'), 1500)

  } catch (err: any) {
    console.error('Error guardando comercio:', err)
    errorMsg.value = err?.message || 'Ocurrió un error al guardar. Intentá de nuevo.'
  } finally {
    saving.value = false
  }
}
